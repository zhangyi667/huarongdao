...
public class MoveHorizonBuilder {
    public static MoveHorPiece build(String val, Snapshot snapshot, Direction direction) {
        switch (direction) {
            ...
            case RIGHT: {
                if (val.equals(PointConstant.Point.HOR_PIECE_LEFT))
    ...
    private static boolean hasEnoughSpace(String val, Snapshot snapshot) {
        return (val.equals(PointConstant.Point.HOR_PIECE_LEFT) && snapshot.getMaps().get(snapshot.getSpace1().right()).equals(PointConstant.Point.SPACE)) ||
                (val == PointConstant.Point.HOR_PIECE_RIGHT && snapshot.getMaps().get(snapshot.getSpace1().left()).equals(PointConstant.Point.SPACE));
    }
}
